...
export default function LoginForm({ initialError }: Props) {
  ...
  const handleSubmit = async (e: React.SyntheticEvent<HTMLFormElement>) => {
    ...
    setStatus("loading");
    const { error } = await authClient.signIn.email({ email: em, password: pw });
  ...
  return (
    ...
    >
      ...
      <label htmlFor="login-email" className="text-sm font-semibold text-[#3d3530]">
        Email Address
      </label>
      ...
      <label htmlFor="login-password" className="text-sm font-semibold text-[#3d3530]">
        Password
      </label>
      <div className="flex items-center mt-2 mb-5 h-11 pl-3 border border-[#0d0b18]/15 rounded-full focus-within:ring-2 focus-within:ring-[#d4a853]/50 focus-within:border-[#d4a853] transition-all overflow-hidden bg-white">
      ...
      <p className="text-sm text-center text-[#7a6f66] mt-6">
        ...
        <a href="/signup" className="text-[#d4a853] font-semibold hover:underline">
          Sign up
        </a>
      </p>
    </form>
